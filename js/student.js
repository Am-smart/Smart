        ...
        <div style="flex:1">
          ...
          <div class="flex-between">
            ${enrolled ?
              ...
              `<button class="button w-auto small" onclick="enroll('${escapeAttr(c.id)}', this)">Enroll Now</button>`
            }
          </div>
        </div>
      </div>
...
async function enroll(courseId, btn) {
  ...
  try {
    ...
    await UI.alert('Successfully enrolled!', 'Enrollment Success');
    renderCourses();
  } catch (e) {
    UI.alert('Enrollment failed: ' + e.message, 'Error');
...
async function showLesson(lessonId, courseId, fromMyCourses = false) {
  ...
  SupabaseDB.saveLessonCompletion({
    student_email: user.email,
    lesson_id: lessonId,
    completed_at: new Date().toISOString()
  }).then(() => {
    SupabaseDB.updateCourseProgress(courseId, user.email);
  }).catch(e => console.warn('Failed to save lesson completion:', e));
...
async function renderAssignments(){
  ...
  try {
    ...
    if(!user || user.role!=='student'){
        await UI.alert('Please login as a student to access this page.', 'Access Denied');
  ...
  mine.forEach(a => {
    ...
    const isLateDisallowed = isPastDue && !a.allow_late_submissions;
    ...
    } else if (isLateDisallowed) {
      statusHtml = `<span class="badge badge-inactive">CLOSED</span>`;
    } else if (isOverdue) {
    ...
    row.innerHTML = `
      ...
      <td>
        ...
        ${isOverdue ? `<div class="small danger-text">(${isLateDisallowed ? 'Closed' : 'Overdue'})</div>` : ''}
      </td>
      <td>${statusHtml}</td>
      ...
      <td>
        <div class="flex gap-5">
          ${!submission ?
            (isLateDisallowed ?
              `<button class="button small w-auto secondary" disabled>Closed</button>` :
...
function closeAssignmentForm() {
  ...
  if (window.AntiCheat) AntiCheat.disable();
}
...
async function renderDashboardOverview() {
  ...
  `;
    ...
    container.innerHTML = `<div class="stat-card danger"><h3>Error Loading Overview</h3><button class="button w-auto mt-10" onclick="renderDashboardOverview()">Retry</button></div>`;
  }
}
...
function showReplyForm(parentId, courseId) {
...
async function postDiscussion(courseId, parentId = null) {
  ...
  try {
    ...
    UI.alert('Failed to post message: ' + e.message, 'Error');
    if (btn && btn.tagName === 'BUTTON') { btn.disabled = false; btn.textContent = parentId ? 'Reply' : 'Post'; }
  }
}
...
async function deleteStudentDiscussion(id, courseId) {
  if (!await UI.confirm('Are you sure you want to delete this message?', 'Delete Discussion')) return;
  try {
...
async function addPlannerItem() {
  const btn = document.querySelector('button[onclick="addPlannerItem()"]');
  const user = await SessionManager.getCurrentUser();
  ...
  try {
    ...
    UI.alert('Failed to add task: ' + e.message, 'Error');
...
async function saveNotificationSettings() {
  ...
  try {
    ...
    UI.showNotification('Preferences saved');
  } catch (e) {
    UI.alert('Failed to save settings: ' + e.message, 'Error');
  } finally {
          ...
          <div class="card">
            ...
            <div class="mt-20">
                ${!canAttempt ?
                    ...
                    (isNotStarted ?
                        `<div class="badge badge-warn w-100 text-center">Available from ${startAt.toLocaleString()}</div>` :
                        (isEnded ?
                            '<div class="badge badge-inactive w-100 text-center">Quiz Closed</div>' :
                            `<button class="button w-auto small px-20" onclick="startQuiz('${q.id}', this)">Start New Attempt</button>`
                        )
                    )
                }
            </div>
...
async function startQuiz(quizId, btn) {
  ...
  if (quiz.start_at && now < new Date(quiz.start_at)) {
    await UI.alert(`This quiz is not available until ${new Date(quiz.start_at).toLocaleString()}.`, 'Quiz Not Available');
    renderQuizzes();
    return;
  }
  if (quiz.end_at && now > new Date(quiz.end_at)) {
    await UI.alert('This quiz is now closed.', 'Quiz Closed');
    renderQuizzes();
    return;
  }
  ...
  if (draft && !await UI.confirm('You have an unfinished attempt. Resume it?', 'Resume Quiz')) {
...
async function submitQuiz(isAuto = false) {
  ...
  const violationCount = AntiCheat.getViolationCount();
  AntiCheat.disable();
  ...
  await renderQuizzes();
  const alertMsg = isAuto ? `Time is up! Your quiz was submitted automatically. Your score: ${percentage}%` : `Quiz submitted! Your score: ${percentage}%`;
  await UI.alert(alertMsg, isAuto ? 'Time Expired' : 'Quiz Completed');
}
...
async function requestRegrade(assignmentId) {
    const btn = document.querySelector('button[onclick^="requestRegrade"]');
    const reason = document.getElementById('regradeReason').value;
    if (!reason) return UI.alert('Please provide a reason.', 'Validation Error');
    ...
    try {
        ...
        await UI.alert('Regrade request submitted!', 'Success');
        viewFeedback(assignmentId);
    } catch (e) {
        UI.alert('Failed to submit regrade request: ' + e.message, 'Error');
...
async function deleteSubmissionById(assignmentId, studentEmail) {
  if (await UI.confirm('Are you sure you want to delete this submission?', 'Delete Submission')) { try { await SupabaseDB.deleteSubmission(assignmentId, studentEmail); renderAssignments(); } catch (e) { UI.alert('Error deleting submission: ' + e.message, 'Error'); } }
}
async function submitAssignment(assignmentId, studentEmail) {
  ...
  try {
    ...
    if (await SupabaseDB.saveSubmission(submission)) {
      ...
      await UI.alert('Assignment submitted successfully!', 'Success');
      renderAssignments();
    }
  } catch (e) {
    console.error('Submission failed:', e);
    UI.alert(`Submission failed: ${e.message || 'Unknown error'}. ${e.details || ''}`, 'Error');
  } finally {
...
async function renderAntiCheat() {
  ...
  try {
    const user = await SessionManager.getCurrentUser();
    const [subs, quizSubs] = await Promise.all([
      SupabaseDB.getSubmissions(null, user.email),
      SupabaseDB.getQuizSubmissions(null, user.email)
    ]);
    ...
    content.innerHTML = `
      <div class="card">
        <h2 class="m-0">Security Record</h2>
        <p class="small mt-5">This page shows the violation counts recorded during your recent assessments. We use these metrics to ensure a fair environment for everyone.</p>
      </div>
        ...
        <table>
          ...
          <tbody>
            ${allAssessments.map(s => {
              ...
              return `
                <tr>
                  <td><div class="bold small">${escapeHtml(s.title || 'Unknown')}</div></td>
                  <td><span class="badge">${s.type}</span></td>
                  <td><span class="badge ${violations > 0 ? 'badge-lock' : ''}">${violations}</span></td>
                  <td><span class="badge ${s.status === 'graded' ? 'badge-active' : ''}">${s.status.toUpperCase()}</span></td>
    ...
    console.error('Anti-cheat record error:', error);
    content.innerHTML = `<div class="card danger-border"><h3>Error Loading Records</h3><p class="small">${escapeHtml(error.message)}</p></div>`;
  }
}
...
async function navigateToPage(pageId) {
  ...
  studentNav.querySelectorAll('button').forEach(b => {
  ...
  else if(pageId === 'my-courses') renderMyCourses();
  ...
  else if(pageId === 'dashboard') renderDashboardOverview();
  else if(pageId === 'progress') renderProgress();
...
async function setupStudentRealtime() {
  if (studentChannels.length) return;
  const user = await SessionManager.getCurrentUserSync();
  if (!user) return;

  // 1. Submissions monitoring (for grades)
  const subChannel = SupabaseDB.subscribe('submissions', (payload) => {
    const page = new URLSearchParams(window.location.search).get('page');
    if (page === 'grades' || page === 'dashboard' || page === 'assignments') {
        navigateToPage(page);
    }
  }, `student_email=eq.${user.email}`);
  studentChannels.push(subChannel);

  // 2. Live Classes monitoring
  const liveChannel = SupabaseDB.subscribe('live_classes', (payload) => {
    const page = new URLSearchParams(window.location.search).get('page');
    if (page === 'live') {
        renderLiveClasses();
    }
  });
  studentChannels.push(liveChannel);

  // 3. Discussions monitoring
  const discChannel = SupabaseDB.subscribe('discussions', (payload) => {
    ...
    if (page === 'discussions') {
        ...
        if (courseId) viewStudentDiscussions(courseId);
        else renderDiscussions();
    }
  });
  studentChannels.push(discChannel);
}
