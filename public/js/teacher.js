...
async function loadAndEditCourse(id) {
    try {
        ...
        UI.alert('Error loading course: ' + e.message, 'Error');
    }
}
...
function showCourseForm(course = null) {
  ...
  document.getElementById('courseForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      UI.alert('Error saving course: ' + err.message, 'Error');
      if (btn) { btn.disabled = false; btn.textContent = isEdit ? 'Update Course' : 'Create Course'; }
    }
  });
}
...
function showLessonForm(courseId, lesson = null) {
  ...
  document.getElementById('lessonForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      UI.alert('Error saving lesson: ' + err.message, 'Error');
      if (btn) { btn.disabled = false; btn.textContent = isEdit ? 'Update Lesson' : 'Save Lesson'; }
    }
  });
}
...
async function deleteLessonById(id, courseId) {
  if (await UI.confirm('Are you sure you want to delete this lesson?', 'Delete Lesson')) {
    try {
      ...
      UI.alert('Error deleting lesson: ' + e.message, 'Error');
    }
  }
}
async function deleteCourseById(id) {
  if (await UI.confirm('Are you sure you want to delete this course?', 'Delete Course')) {
    try {
      await SupabaseDB.deleteCourse(id);
      renderCourses();
    } catch (e) {
      UI.alert('Error deleting course: ' + e.message, 'Error');
    }
  }
}
        ...
        gradingHtml += `
          <div class="card">
            ...
            <div class="p-0 mt-10" style="overflow-x:auto">
                <table>
                  <thead><tr><th>Student</th><th>Submitted</th><th>Status</th><th>Violations</th><th>Action</th></tr></thead>
                  <tbody>
                    ${pendingSubmissions.map(s => {
                      ...
                      <tr>
                        ...
                        <td><span class="badge ${violations > 0 ? 'badge-lock' : ''}">${violations}</span></td>
    ...
    <div class="card">
      ...
      <div class="p-0 mt-15" style="overflow-x:auto">
          <table>
            ...
            <tbody>
              ${students.map(s => {
                const studentEnrollments = enrollments.filter(e => e.student_email === s.email);
                return `
                <tr>
                  ...
                  <td>
                    <div class="flex gap-5">
                      ...
                      <button class="button danger small w-auto" onclick="showUnenrollOptions('${escapeAttr(s.email)}')">Unenroll</button>
                    </div>
                  </td>
                </tr>
...
async function showUnenrollOptions(studentEmail) {
  const user = await SessionManager.getCurrentUser();
  const courses = await SupabaseDB.getCourses(user.email);
  const courseIds = courses.map(c => c.id);
  const allEnrollments = await SupabaseDB.getEnrollmentsByCourses(courseIds);

  const studentEnrollments = allEnrollments.filter(e => e.student_email === studentEmail);
  const enrolledCourses = courses.filter(c => studentEnrollments.some(e => e.course_id === c.id));
  ...
  area.innerHTML = `
    <div class="card danger-border">
      <h3 class="m-0">Unenroll Student: ${escapeHtml(studentEmail)}</h3>
      <p class="small mt-10">Select the course to unenroll this student from. This action is irreversible.</p>
      <label class="mt-15">Course</label>
      <select id="unenrollCourseId">${enrolledCourses.map(c => `<option value="${escapeAttr(c.id)}">${escapeHtml(c.title)}</option>`).join('')}</select>
      <div class="flex gap-10 mt-15">
        <button class="button danger w-auto px-30" id="unenrollBtn" onclick="unenrollStudent('${escapeAttr(studentEmail)}')">Unenroll Now</button>
        <button class="button secondary w-auto px-30" onclick="document.getElementById('certFormArea').classList.add('hidden')">Cancel</button>
      </div>
    </div>
  `;
...
async function unenrollStudent(studentEmail) {
  const courseId = document.getElementById('unenrollCourseId').value;
  if (!courseId) return;

  if (await UI.confirm(`Are you sure you want to unenroll ${studentEmail} from this course?`, 'Confirm Unenrollment')) {
    ...
    try {
      ...
      UI.alert('Unenrollment failed: ' + e.message, 'Error');
      if (btn) { btn.disabled = false; btn.textContent = 'Unenroll Now'; }
    }
  }
}
...
async function issueCert(studentEmail) {
  ...
  try {
    ...
    await UI.alert('Certificate issued successfully!', 'Success');
    renderStudents();
    const area = document.getElementById('certFormArea');
    if (area) area.style.display = 'none';
  } catch (e) {
    console.error('Cert Issue error:', e);
    UI.alert('Error issuing certificate: ' + e.message, 'Error');
  } finally {
      ...
      <form id="assignmentForm">
        ...
        <div class="grid-2 mt-10">
          ...
          <div class="flex-center-y" style="height:100%">
            <label class="flex-center-y gap-10 m-0"><input type="checkbox" id="antiCheatEnabled" ${isEdit && assignment.anti_cheat_enabled ? 'checked' : ''} class="w-auto m-0"> Enable Anti-Cheat Protection</label>
          </div>
        </div>
        <div class="mt-20">
  ...
  document.getElementById('assignmentForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const assignmentData = {
        ...
        anti_cheat_enabled: document.getElementById('antiCheatEnabled').checked,
        teacher_email: user.email,
      ...
      if (result) { UI.showNotification('Assignment saved successfully', 'success'); if (selCourseId && !assignment) editCourse(selCourseId); else renderAssignments(); }
    } catch (err) {
      UI.alert('Error saving assignment: ' + err.message, 'Error');
      if (btn) { btn.disabled = false; btn.textContent = isEdit ? 'Update Assignment' : 'Create Assignment'; }
    }
  });
}
...
async function deleteAssignmentById(id, courseId = null) {
  if (await UI.confirm('Are you sure you want to delete this assignment?', 'Delete Assignment')) { await SupabaseDB.deleteAssignment(id); if (courseId) editCourse(courseId); else renderAssignments(); }
}
    ...
    <div class="card">
      ...
      <div class="flex-between mt-10">
          ...
          <div class="flex gap-10">
            <p class="small"><strong>Violations:</strong> <span class="badge ${submission.violation_count > 0 ? 'badge-lock' : ''}">${submission.violation_count || 0}</span></p>
  ...
  document.getElementById('gradingForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (await SupabaseDB.saveSubmission(updatedSubmission)) {
        UI.showNotification('Submission graded', 'success');
        renderGrading();
      }
    } catch (e) {
      UI.alert('Error saving grade: ' + e.message, 'Error');
    }
  });
  } catch (error) {
...
async function viewCourseDiscussions(courseId) {
  ...
  container.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="flex gap-10">
        <input type="text" id="discInput" data-course-id="${escapeAttr(courseId)}" placeholder="Start a new thread..." class="m-0">
...
function showTeacherReplyForm(parentId, courseId) {
...
async function postTeacherDiscussion(courseId, parentId = null) {
  ...
  try {
    ...
    UI.alert('Error posting message: ' + e.message, 'Error');
  }
}
...
async function saveDiscussionEdit(id, courseId) {
  ...
  try {
    ...
    UI.alert('Error updating: ' + e.message, 'Error');
  }
}
...
async function deleteDiscussion(id, courseId) {
  ...
  try {
    ...
    UI.alert('Error deleting: ' + e.message, 'Error');
  }
}

function toggleTeacherAssignmentType(select) {
...
function toggleQuizOptions(select) {
...
async function awardBadge(badgeId) {
  const btn = document.querySelector(`button[onclick="awardBadge('${badgeId}')"]`);
  ...
  try {
    ...
    UI.showNotification('Badge awarded!', 'success');
    if (btn) { btn.disabled = false; btn.textContent = 'Award'; }
  } catch (e) {
    UI.alert('Error awarding badge: ' + e.message, 'Error');
  }
}
      ...
      try {
        ...
        if (selPattern !== 'none' && selUrl) {
            ...
            if (course) {
                ...
                await SupabaseDB.saveCourse(course).catch(err => console.error('Failed to save course metadata', err));
            }
        }
        ...
        UI.alert('Error saving live class: ' + err.message, 'Error');
...
function startLiveClassTimer(id, endAt) {
    ...
    liveClassTimer = setInterval(async () => {
        const now = Date.now();
        if (now >= endTime) {
            clearInterval(liveClassTimer);
            if (await UI.confirm('Scheduled class time has reached. Do you want to extend by 15 minutes? Press Cancel to end class.', 'Class Time Over')) {
...
async function deleteLiveClass(id) {
  if (await UI.confirm('Are you sure you want to cancel this class?', 'Cancel Class')) {
...
async function viewAttendance(classId) {
  ...
  const list = att.map(a => `${escapeHtml(a.student_email)}: ${Math.floor(a.duration / 60)} mins (${a.is_present ? 'Present' : 'Absent'})`).join('\n') || 'No records yet.';
  UI.alert(list, 'Attendance Records');
}
...
async function showQuizForm(quiz = null) {
  ...
  container.innerHTML = `
    <div class="card">
      ...
      <form id="quizForm" class="mt-20">
        ...
        <div class="grid-2 mt-10">
          <div><label class="small">Max Points:</label><input type="number" id="quizTotalPoints" value="0" readonly style="background:#f0f0f0"></div>
        ...
        <div class="grid-2 mt-10">
          ...
          <div class="flex-center-y" style="height:100%">
            <label class="flex-center-y gap-10 m-0"><input type="checkbox" id="antiCheatEnabled" ${isEdit && quiz.anti_cheat_enabled ? 'checked' : ''} class="w-auto m-0"> Enable Anti-Cheat Protection</label>
          </div>
        </div>
        <div class="grid-2 mt-10">
          <div><label class="small">Available From (optional):</label><input type="datetime-local" id="quizStartAt" value="${isEdit && quiz.start_at ? new Date(quiz.start_at).toISOString().slice(0, 16) : ''}"></div>
          <div><label class="small">Available Until (optional):</label><input type="datetime-local" id="quizEndAt" value="${isEdit && quiz.end_at ? new Date(quiz.end_at).toISOString().slice(0, 16) : ''}"></div>
        </div>
        <div class="mt-20">
          <div class="flex-between">
            <h3 class="m-0">Questions</h3>
  ...
  document.getElementById('quizForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      UI.alert('Error saving quiz: ' + err.message, 'Error');
      if (btn) { btn.disabled = false; btn.textContent = isEdit ? 'Update Quiz' : 'Save Quiz'; }
    }
  });
}
...
async function deleteQuizById(id) {
  if (await UI.confirm('Are you sure you want to delete this quiz?', 'Delete Quiz')) {
    try {
      ...
      UI.alert('Error deleting quiz: ' + e.message, 'Error');
    }
  }
}
...
async function viewQuizResults(quizId) {
  ...
  container.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="p-0 mt-15" style="overflow-x:auto">
          <table>
            <thead><tr><th>Student</th><th>Score</th><th>Violations</th><th>Points</th><th>Submitted</th><th>Action</th></tr></thead>
            <tbody>
              ${subs.filter(s => s.status === 'submitted').map(s => `
                <tr>
                  <td>${escapeHtml(s.student_email)}</td>
                  <td>${s.score !== null ? s.score + '%' : '<span class="warning-text bold">Pending</span>'}</td>
                  <td><span class="badge ${s.violation_count > 0 ? 'badge-lock' : ''}">${s.violation_count || 0}</span></td>
...
async function gradeQuizSubmission(submissionId, quizId) {
  ...
  container.innerHTML = `
    ...
    <div class="card">
      <div class="flex-between">
        ...
        <p class="small m-0"><strong>Violations:</strong> <span class="badge ${submission.violation_count > 0 ? 'badge-lock' : ''}">${submission.violation_count || 0}</span></p>
      </div>
  ...
  document.getElementById('quizGradingForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      await UI.alert('Quiz graded successfully!', 'Success');
      viewQuizResults(quizId);
    } catch (err) {
      UI.alert('Error saving grade: ' + err.message, 'Error');
    }
  });
}
...
async function renderAntiCheat() {
  ...
  try {
    const user = await SessionManager.getCurrentUser();
    const [subs, quizSubs, logs] = await Promise.all([
      SupabaseDB.getSubmissions(null, null, user.email),
      SupabaseDB.getQuizSubmissions(null, null, user.email),
      SupabaseDB.getAntiCheatLogs()
    ]);

    const allAssessments = [
      ...subs.map(s => ({ ...s, type: 'Assignment', title: s.assignments?.title })),
      ...quizSubs.map(s => ({ ...s, type: 'Quiz', title: s.quizzes?.title }))
    ].filter(s => s.status === 'submitted' || s.status === 'graded');

    allAssessments.sort((a,b) => new Date(b.submitted_at) - new Date(a.submitted_at));

    content.innerHTML = `
      <div class="card">
        <h2 class="m-0">Security Overview</h2>
        <p class="small mt-5">Review violation counts for recent student assessments. Higher counts may indicate attempted cheating.</p>
      </div>
      <div class="card p-0 mt-20" style="overflow-x:auto">
        <table>
          ...
          <tbody>
            ${allAssessments.map(s => {
              const violations = s.violation_count || 0;
              return `
                <tr>
                  <td><div class="bold small">${escapeHtml(s.student_email)}</div></td>
                  <td><div class="small">${escapeHtml(s.title || 'Unknown')}</div></td>
                  <td><span class="badge">${s.type}</span></td>
                  <td><span class="badge ${violations > 0 ? 'badge-lock' : ''}">${violations}</span></td>
                  <td class="small">${new Date(s.submitted_at).toLocaleString()}</td>
                </tr>
              `;
            }).join('') || '<tr><td colspan="5" class="empty">No assessment submissions found.</td></tr>'}
          </tbody>
        </table>
      </div>
    `;
  } catch (error) {
    console.error('Anti-cheat overview error:', error);
    content.innerHTML = `<div class="card danger-border"><h3>Error Loading Overview</h3><p class="small">${escapeHtml(error.message)}</p></div>`;
  }
}
...
function navigateToPage(pageId) {
  ...
  teacherNav.querySelectorAll('button').forEach(b => {
    if (b.dataset.page === pageId) b.classList.add('active');
    else b.classList.remove('active');
  });

  if(pageId === 'dashboard') renderDashboard();
  else if(pageId === 'courses') renderCourses();
  else if(pageId === 'materials') renderMaterials();
  else if(pageId === 'assignments') renderAssignments();
  else if(pageId === 'grading') renderGrading();
  else if(pageId === 'gradebook') renderGradeBook();
  else if(pageId === 'students') renderStudents();
  else if(pageId === 'discussions') renderDiscussions();
  else if(pageId === 'badges') renderBadges();
  else if(pageId === 'quizzes') renderQuizzes();
  else if(pageId === 'live') renderLiveClasses();
  else if(pageId === 'anti-cheat') renderAntiCheat();
...
window.showTeacherReplyForm = showTeacherReplyForm;
window.toggleTeacherAssignmentType = toggleTeacherAssignmentType;
...
window.showUnenrollOptions = showUnenrollOptions;
...
async function saveMaterial() {
  ...
  if (!title || !url) return UI.alert('Title and file required', 'Validation Error');
  ...
  try {
    ...
    UI.alert('Save failed: ' + e.message, 'Error');
    if (btn) { btn.disabled = false; btn.textContent = 'Save Material'; }
  }
}
...
async function deleteMaterial(id) {
  if (await UI.confirm('Are you sure you want to delete this material?', 'Delete Material')) {
    try {
      ...
      UI.alert('Delete failed: ' + e.message, 'Error');
    }
  }
}
...
async function setupTeacherRealtime() {
  if (teacherChannels.length) return;
  const user = await SessionManager.getCurrentUser();
  if (!user) return;

  // 1. Submissions monitoring (for grading queue)
  const subChannel = SupabaseDB.subscribe('submissions', (payload) => {
    const page = new URLSearchParams(window.location.search).get('page');
    if (page === 'grading' || page === 'dashboard' || page === 'gradebook') {
        navigateToPage(page);
    }
  });
  teacherChannels.push(subChannel);

  // 2. Discussions monitoring
  const discChannel = SupabaseDB.subscribe('discussions', (payload) => {
    const page = new URLSearchParams(window.location.search).get('page');
    if (page === 'discussions') {
        // We might want more targeted refresh here, but for now simple refresh is safest
        const courseId = document.getElementById('discInput')?.getAttribute('data-course-id');
        if (courseId) viewCourseDiscussions(courseId);
        else renderDiscussions();
    }
  });
  teacherChannels.push(discChannel);
}
