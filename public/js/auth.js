...
const Auth = {
    ...
    async init() {
        ...
        // Start maintenance banner Realtime subscription
        this.updateMaintBanners(m);
        SupabaseDB.subscribe('maintenance', () => this.updateMaintBanners());
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (signupForm) {
        signupForm.addEventListener('submit', async (e) => {
            ...
            if (isActiveMaintenance(m)) {
                ...
                await UI.alert(`System is currently undergoing maintenance. Signups are disabled until ${untilStr}.`, 'Maintenance Mode');
                return;
            }
            const upcoming = getUpcomingMaintenance(m);
            if(upcoming){ await UI.alert(`Upcoming system maintenance: ${new Date(upcoming.startAt).toLocaleString()}`, 'Maintenance Notice'); }
            ...
            await UI.alert(`Welcome ${fullName}! Your ${role} account has been created.`, 'Registration Success');
        ...
        loginForm.addEventListener('submit', async (e) => {
            ...
            if (isActiveMaintenance(m)) {
                ...
                if (!allow) {
                    const untilTs = getActiveMaintenanceEnd(m);
                    const untilStr = untilTs ? new Date(untilTs).toLocaleString() : 'the scheduled end time';
                    await UI.alert(`System is currently undergoing maintenance. Only admin login allowed until ${untilStr}.`, 'Maintenance Mode');
                    return;
                }
            }
            const upcoming = getUpcomingMaintenance(m);
            ...
            await UI.alert(`Welcome back ${user.full_name}!`, 'Login Success');
    ...
    if (resetForm) {
        resetForm.addEventListener('submit', async (e) => {
            ...
            await UI.alert('Password reset request submitted. Admin will review it.', 'Request Submitted');
            Auth.showLogin();
        });
    }
    ...
    if (newPasswordForm) {
        newPasswordForm.addEventListener('submit', async (e) => {
            ...
            await UI.alert('Password successfully reset. You can now login with your new password.', 'Reset Success');
            Auth.showLogin();
        });
    }
});
