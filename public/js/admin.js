...
async function toggleUserStatus(email, currentStatus) {
  ...
  } catch (e) { UI.alert('Error: ' + e.message, 'Operation Failed'); }
}
...
async function exportUsersCSV() {
  ...
  if (listToExport.length === 0) return UI.alert('No users to export', 'Export Error');
...
async function broadcastNotif() {
  const btn = document.querySelector('button[onclick="broadcastNotif()"]');
  ...
  if (!title || !msg) return UI.alert('Title and message required', 'Validation Error');
  ...
  try {
    ...
    await UI.alert(`Broadcast sent successfully.`, 'Broadcast Sent');
    document.getElementById('bcTitle').value = '';
    document.getElementById('bcMsg').value = '';
    renderDashboard();
  } catch (e) {
    UI.alert('Broadcast failed: ' + e.message, 'Error');
...
async function approveReset(email) {
  const btn = document.querySelector(`button[onclick="approveReset('${email}')"]`);
  ...
  try {
    ...
    if (user && user.reset_request) {
      ...
      if (await SupabaseDB.saveUser(user)) {
        await UI.alert(`Reset request approved. Temporary password: ${tempPassword}\n\nPLEASE COPY THIS NOW. IT WILL NOT BE SHOWN AGAIN.`, 'Reset Approved');
        renderResets();
        updateSidebarBadges();
      }
    }
  } catch (e) {
    UI.alert('Error approving reset: ' + e.message, 'Error');
    if (btn) { btn.disabled = false; btn.textContent = 'Approve'; }
  }
}

async function denyReset(email) {
  const reason = await UI.prompt("Enter denial reason:", "Deny Reset Request");
  if (reason !== null) {
    try {
      const user = await SupabaseDB.getUser(email);
      if (user && user.reset_request) {
        ...
        if (await SupabaseDB.saveUser(user)) {
          UI.showNotification('Reset request denied');
          renderResets();
          updateSidebarBadges();
        }
      }
    } catch (e) {
      UI.alert('Error denying reset: ' + e.message, 'Error');
    }
  }
}
...
async function renderMaintenance() {
  ...
  document.getElementById('maintenanceForm').addEventListener('submit', async (e) => {
    ...
    if (await SupabaseDB.saveMaintenance(maintenance)) { UI.showNotification('Maintenance settings saved', 'success'); renderMaintenance(); }
  });
  } catch (error) {
...
async function executeCleanup() {
  if (!await UI.confirm('Are you sure? This action is irreversible.', 'System Cleanup')) return;
  try {
    const [users, courses] = await Promise.all([SupabaseDB.getUsers(), SupabaseDB.getCourses()]);
    const inactiveUsers = users.filter(u => !u.active);
    const draftCourses = courses.filter(c => c.status === 'draft');
    ...
    await UI.alert(`Cleanup successful: ${inactiveUsers.length} users and ${draftCourses.length} courses removed.`, 'Cleanup Complete');
    renderManagement();
  } catch (e) {
    UI.alert('Cleanup failed: ' + e.message, 'Cleanup Error');
  }
}
...
async function importBackup(event) {
  ...
  reader.onload = async (e) => {
    try {
      ...
      if (await UI.confirm(`Restore ${data.users?.length || 0} users and ${data.courses?.length || 0} courses? This will overwrite existing records with same IDs.`, 'System Restore')) {
        ...
        await UI.alert('System Restore completed successfully.', 'Restore Success');
        renderManagement();
      }
    } catch (err) {
      console.error('Restore error:', err);
      UI.alert('Failed to restore backup: ' + err.message, 'Restore Error');
    }
  };
...
function showUserForm(user = null) {
  ...
  document.getElementById('userForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!fullName) return UI.alert('Full name is required.', 'Validation Error');
      if (!isValidEmail(email)) return UI.alert('Please enter a valid email address.', 'Validation Error');

      let hashedPassword = isEdit ? user.password : '';
      if (password) {
        if (!isStrongPassword(password)) {
          await UI.alert('Password must be 8+ chars, include upper, lower, number, and special char.', 'Password Weak');
          return;
        }
      ...
      if (isEdit && user.email !== userData.email) {
          if (await SupabaseDB.updateUserEmail(user.email, userData.email, userData)) {
              UI.showNotification('User updated (including email)');
              renderUsers();
          }
      } else {
          if (await SupabaseDB.saveUser(userData)) {
            UI.showNotification(isEdit ? 'User updated' : 'User created');
            renderUsers();
          }
      }
    } catch (err) {
      UI.alert('Error saving user: ' + err.message, 'Error');
      const btn = e.target.querySelector('button[type="submit"]');
...
function showAddScheduleForm() {
  ...
  document.getElementById('scheduleForm').addEventListener('submit', async (e) => {
    ...
    } catch (err) { UI.alert('Failed to add schedule: ' + err.message, 'Error'); }
  });
}
...
function setupAdminRealtime() {
  ...
  adminRealtimeChannel = SupabaseDB.subscribe('users', (payload) => {
    updateSidebarBadges();
