  ...
  return (
    <div className="space-y-8">
      {enrollments.length > 0 && (
        <StudyTimer userId={user.email} courses={enrollments.map(e => e.courses).filter(Boolean) as Course[]} />
      )}
              ...
              {enrollments.length > 0 ? (
                  <div className="space-y-4">
                      {enrollments.slice(0, 3).map(e => (
                          <div key={e.course_id + e.student_id} className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
