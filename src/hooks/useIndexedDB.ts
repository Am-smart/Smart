        ...
        if (role === 'student') {
            ...
            if (quizzes.data) await setCache('all_quizzes', quizzes.data);
             ...
             const [courses, assignments, quizzes] = await Promise.all([
                client.from('courses').select('*').eq('teacher_id', userEmail),
                client.from('assignments').select('*, courses(*)').eq('teacher_id', userEmail),
                client.from('quizzes').select('*, courses(*)').eq('teacher_id', userEmail)
            ]);
