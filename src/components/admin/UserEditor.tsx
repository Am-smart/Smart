        ...
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center z-50 p-4">
            <div className="bg-white w-full max-w-xl rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
                <header className="p-8 border-b bg-slate-50 flex justify-between items-center">
                    <h2 className="text-2xl font-bold text-slate-900">{user?.email ? 'Edit User' : 'Add New User'}</h2>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    ...
                    <div>
                        <label className="block text-sm font-bold text-slate-700 uppercase mb-3 tracking-wide">{user?.email ? 'New Password (Optional)' : 'Password'}</label>
                        <input
                            type="password" required={!user?.email}
                    ...
                    <footer className="pt-8 border-t flex justify-between gap-4">
                        ...
                        <button type="submit" disabled={isSaving} className="btn-primary flex-1 py-4">
                            {isSaving ? 'Saving...' : user?.email ? 'Update User' : 'Create User'}
                        </button>
                    </footer>
                </form>
            </div>
