...
export const Modal: React.FC<ModalProps> = ({ title, onClose, children, footer, maxWidth = 'max-w-2xl' }) => {
    return (
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center z-50 p-2 md:p-4 animate-in fade-in duration-200">
            <div className={`bg-white w-full ${maxWidth} rounded-2xl md:rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in duration-300 flex flex-col max-h-[90vh] md:max-h-[95vh]`}>
                ...
                {footer && (
                    <footer className="p-4 md:p-8 bg-slate-50 border-t shrink-0">
                        {footer}
                    </footer>
                )}
            </div>
