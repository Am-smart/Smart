...
export const LandingHeader: React.FC<HeaderProps> = ({ onSignIn, onGetStarted }) => {
  return (
    <header className="fixed top-0 left-0 right-0 h-[70px] bg-white flex justify-between items-center px-[5%] z-[1000] shadow-[0_2px_10px_rgba(0,0,0,0.05)]">
      ...
      <button onClick={onGetStarted} className="bg-[#2563eb] text-white px-4 md:px-6 py-[0.6rem] rounded-lg font-semibold transition-all hover:bg-[#1d4ed8] hover:-translate-y-0.5 active:translate-y-0 text-sm md:text-base">Get Started</button>
    </header>
  );
};
